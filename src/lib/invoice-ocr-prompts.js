...
PODMIOTY NA FAKTURZE — ZAWSZE OSOBNO (nigdy nie mieszaj):
- seller_name + seller_nip = WYŁĄCZNIE blok „Sprzedawca” / „Wystawca” (kto wystawił fakturę).
- contractor_name + contractor_nip = WYŁĄCZNIE blok „Nabywca” / „Nabywca towaru lub usług” (druga strona transakcji).
- Nie kopiuj danych między tymi polami; nie wstawiaj nazwy sprzedawcy jako kontrahenta ani odwrotnie.
- Pełna nazwa handlowa z właściwego bloku; wielolinijkowe nazwy scal w jeden ciąg; adres (ul./kod) pomiń, jeśli da się oddzielić.
- NIP (PL): 10 cyfr przy właściwym podmiocie — seller_nip tylko dla sprzedawcy, contractor_nip tylko dla nabywcy.
- Flaga is_own_company_seller: true tylko gdy na dokumencie wystawcą jest Twoja firma (FV sprzedaży); nie zmienia reguł pól seller vs contractor — nadal wypełnij oba bloki z dokumentu.
...
- seller_name / contractor_name: jak w regułach głównych — każde pole tylko ze swojej sekcji dokumentu (nie skróty z pieczątek jako jedyne źródło).
- NIP: dokładnie 10 cyfr (PL) przy właściwym podmiocie — bez zamiany NIP sprzedawcy z NIP nabywcy.
